...
function initStore() {
    ...
    wmiOperation.GetLogoLight().then(r=>{
        storeModels.state.OS.LogoLight = r
    }).catch(e=>alert("运行时出现异常，请报告错误"))
    wmiOperation.Gpu.GetGpuMode().then(r=>{
        storeModels.state.OS.GPU.GpuMode = r
    }).catch(e=>alert("运行时出现异常，请报告错误"))
    return storeModels
}
