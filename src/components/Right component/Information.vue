...
const info = store.state.ProbeData as ProbeData
...
async function setLogoLight() {
  if (store.state.OS.LogoLight != ResultState.ON) {
...
async function setSwitchMaxFanSpeed() {
  if (store.state.OS.SwitchMaxFanSpeed != ResultState.ON) {
...
async function setCpuCustomize() {
  if (store.state.OS.Customize!=eumCPUPower.OpenState){
  ...
  <div class="Information">
    <div class="mdui-table-fluid">
      ...
      <table class="mdui-table mdui-table-hoverable">
        ...
        <tr>
          <td>系统模式</td>
          <td>{{ store.state.OS.PerformaceMode }}</td>
          <td>CPU增压</td>
          <td>
            <div @click="setCpuCustomize">
              <label class="mdui-switch">
                <input :checked="Number(store.state.OS.Customize)" type="checkbox"/>
        ...
        <tr>
          ...
          <td>
            <div @click="setSwitchMaxFanSpeed">
              <label class="mdui-switch">
                <input :checked="Number(store.state.OS.SwitchMaxFanSpeed)" type="checkbox"/>
                <i class="mdui-switch-icon"></i>
              </label>
            </div>
          </td>
          ...
          <td>
            <div @click="setLogoLight">
              <label class="mdui-switch">
                <input :checked="store.state.OS.LogoLight" type="checkbox"/>
                <i class="mdui-switch-icon"></i>
              </label>
            </div>
          </td>
      ...
      <table class="mdui-table mdui-table-hoverable">
        ...
        <tr >
          <td>CPU温度</td>
          <td>{{ store.state.OS.CPU.temperature }} °C</td>
          <td>CPU风扇转速</td>
          <td>{{ store.state.OS.CPU.CPUFanSpeed }}</td>
        </tr>
        <tr >
          <td>GPU温度</td>
          <td>{{ store.state.OS.GPU.temperature }} °C</td>
          <td>GPU风扇转速</td>
          <td>{{ store.state.OS.GPU.GPUFanSpeed }}</td>
        </tr>
        <tr >
          <td>GPU使用率</td>
          <td>{{ store.state.OS.GPU.GpuUsage }} %</td>
          <td>GPU模式</td>
          <td> {{ store.state.OS.GPU.GpuMode? "DiscreteMode" : "HybridMode" }}</td>
        </tr>
        <tr >
          <td>键盘模式</td>
          <td>{{ store.state.OS.KeyboardMode }} </td>
          <td>电源接口</td>
          <td> {{ store.state.OS.AcType }}</td>
        </tr>
        </tbody>
      </table>
    </div>
    <button class="mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple" @click="store.state.Debug=true">
      <p>DevTools</p>
    </button>
  </div>
</template>
